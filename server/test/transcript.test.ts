...
test("validateTranscriptPath rejects empty / non-string input", () => {
  ...
  assert.equal(validateTranscriptPath(undefined as unknown as string), null);
});
...
test("filterEntries removes advisor tool_use and its matching tool_result", () => {
  ...
  const assistantContent = out[1]!.message!.content as Array<{ type: string; text?: string }>;
